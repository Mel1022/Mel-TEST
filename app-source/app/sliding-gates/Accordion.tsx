...
export function Accordion({ items }: { items: { title: string; summary?: string; body: string | React.ReactNode }[] }) {
    ...
    <div className="divide-y divide-steel/10 border border-steel/10 rounded-card overflow-hidden">
      {items.map((item, i) => (
        <div key={i}>
          ...
          {open === i && (
            <div className="px-6 pb-5 text-sm text-steel leading-relaxed bg-surface">
              {item.body}
            </div>
          )}
        </div>
